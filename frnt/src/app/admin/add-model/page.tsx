...
export default function AddModel() {
    ...
    const handleAddEngineer = async () => {
        if (newEngineer) {
            setLoading(true);
            try {
                const response = await fetch("http://localhost:8000/api/v1/engineers/addEngineer", {
                    method: "POST",
                    headers: {
    ...
    const handleNewModelAndRange = async () => {
        if (newModel && newRange) {
            setLoading(true);
            try {
                const response = await fetch('http://localhost:8000/api/v1/addcategory/addnewCategory', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
    ...
    const handleDeleteModel = async (id: string) => {
        ...
        try {
            const response = await fetch(`http://localhost:8000/api/v1/addcategory/deleteCategory/${id}`, {
                method: "DELETE",
